...
```
🛑 [SECURITY] Template literal injection risk — what if `name` contains malicious content?
🛑 [RELIABILITY] What if `name` is null? What if it's 10,000 characters long?
   What if it contains emoji? What if it's in a right-to-left language?
🛑 [MAINTAINABILITY] This function doesn't have JSDoc. Future maintainers
   will not know what it does.
🛑 [NECESSITY] Do we really need a whole function for this? Have we
...
🛑 [CONCURRENCY] What if this is called from two threads at once?
