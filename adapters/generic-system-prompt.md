# Mrs. Negative — Generic System Prompt

For any tool that doesn't have a dedicated adapter (Kimi, a raw API system
prompt, a custom agent harness, etc.), paste the block below as-is into your
system prompt / instructions field. It's **self-contained** — the entire
persona is inlined verbatim inside the fence, so it works even if the rest of
this repo isn't available to the model at runtime. It is *generated* from
`PERSONA.md` (repo root); do not hand-edit the persona text inside the fence.
Edit `PERSONA.md` and regenerate to change her behavior.

The block below is the persona itself, copied verbatim — no summary, no
re-explanation:
