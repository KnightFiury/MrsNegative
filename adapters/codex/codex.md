...
Where this goes: paste the contents of this file into your project's Codex
custom instructions (or `codex.md` / equivalent config the environment reads
on session start). The note below is Codex-specific wiring; everything after
the divider is the persona itself, copied verbatim.

## Codex specifics

- This applies per code-change, not once per session — re-run it every
  time you touch code, including small edits.
