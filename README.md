...
├── benchmark/                        # formal evaluation: 38 cases, rubric, scoring scripts
│   ├── README.md                     # what the benchmark measures, how to run it
│   ├── rubric.md                     # 40-point scoring model; evidence/severity definitions
│   ├── version.md                    # versioned expected answers + bump policy
│   ├── cases/<category>/*.md         # scenario + user request + diff + expected outcome
│   ├── expected/answers.json         # GENERATED by scripts/validate.mjs
│   ├── results/                      # run reports (not committed) — format + template
│   ├── docs/                         # token budget + future evidence-engine design
│   └── scripts/                      # validate.mjs, score.mjs, score.self-test.mjs
├── examples/
...
the calibration layer for an instruction-only repo, and it's run by hand
...
Where `fixtures/` checks *calibration* — does the severity language still
reach the model — `benchmark/` measures *capability*: can Mrs. Negative
actually verify? It holds 38 versioned cases across security, concurrency,
reliability, data integrity, user intent, dependency trust, architecture,
false positives, self-review bias, and lifecycle — including a ~26% slice of
cases that must be answered with **silence**. Each case scores 40 points
across 8 dimensions (detection, severity, evidence, intent,
false-positive avoidance, verification, fix, regression test), and a run
reports precision/recall with **BLOCKING precision measured separately**.

```bash
node benchmark/scripts/validate.mjs           # deterministic case validation + answers.json
node benchmark/scripts/score.self-test.mjs    # deterministic check of the scoring logic
node benchmark/scripts/score.mjs <results>    # score a run (result format in benchmark/results/README.md)
```

The benchmark does not replace the calibration fixtures — run both. Full
details, the versioning policy, and the scoring rubric are in
